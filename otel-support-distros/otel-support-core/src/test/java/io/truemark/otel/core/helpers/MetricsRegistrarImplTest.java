...
class MetricsRegistrarImplTest {
    ...
    void test_registerUpDownCounter_givenUpDownCounterName_shouldRegisterUpDownCounter() {
        LongUpDownCounter upDownCounter = metricRegistrar.registerUpDownCounter("testUpDownCounter", "description", "unit");
        assertNotNull(upDownCounter);
    }

    @ParameterizedTest
    @MethodSource("listProviderOfDoubleValues")
    void test_registerDoubleHistogram_givenDoubleHistogramName_shouldRegisterDoubleHistogram(List<Double> bucketBoundaries) {
        DoubleHistogram histogram = metricRegistrar.registerDoubleHistogram("testHistogram", "description", "unit", bucketBoundaries);
        assertNotNull(histogram);
    }

    @ParameterizedTest
    @MethodSource("listProviderOfLongValues")
    void test_registerLongHistogram_givenLongHistogramName_shouldRegisterLongHistogram(List<Long> bucketBoundaries) {
        LongHistogram histogram = metricRegistrar.registerLongHistogram("testHistogram", "description", "unit", bucketBoundaries);
        assertNotNull(histogram);
    }
    ...
    void test_incrementCounter_givenCounterNameAndValue_shouldIncrementCounter(boolean counterExists) {
        if (counterExists) {
        ...
        if (counterExists) {
            ...
            assertThrows(IllegalStateException.class, () -> metricRegistrar.incrementCounter("testCounter", 1L, Attributes.empty()));
        }
    }

    @ParameterizedTest
    @MethodSource("booleanProvider")
    void test_updateUpDownCounter_givenUpDownCounterNameAndValue_shouldUpdateUpDownCounter(boolean counterExists) {
        if (counterExists) {
        ...
        if (counterExists) {
            ...
            assertThrows(IllegalStateException.class, () -> metricRegistrar.updateUpDownCounter("testUpDownCounter", 1L, Attributes.empty()));
        }
    }

    @ParameterizedTest
    @MethodSource("booleanProvider")
    void test_recordHistogram_givenHistogramNameAndValue_shouldRecordHistogram(boolean histogramExists) {
        if (histogramExists) {
            metricRegistrar.registerDoubleHistogram("testHistogram", "description", "unit", Collections.emptyList());
        }
        if (histogramExists) {
            metricRegistrar.recordHistogram("testHistogram", 1.0, Attributes.empty());
        } else {
            assertThrows(IllegalStateException.class, () -> metricRegistrar.recordHistogram("testHistogram", 1.0, Attributes.empty()));
        }
    }
}